<x-app-layout>
    <x-slot name="header">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Edit Pengguna</h3>
                <p class="text-subtitle text-muted">Formulir untuk mengedit data pengguna</p>
            </div>
    ...
    <section class="section">
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('pengguna.update', $user->id) }}" method="POST" class="form" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <div class="form-group position-relative">
                                ...
                                <input type="text" id="name" class="form-control @error('name') is-invalid @enderror" 
                                    name="name" value="{{ old('name', $user->name) }}" placeholder="Masukkan nama lengkap" required>
                                @error('name')
                        ...
                        <div class="col-md-6">
                            <div class="form-group position-relative">
                                <label for="password" class="form-label fw-bold">Password <small class="text-muted">(Kosongkan jika tidak ingin mengubah)</small></label>
                        ...
                        <div class="col-12">
                            <div class="form-group position-relative">
                                ...
                                <div class="image-preview mt-3" id="imagePreview">
                                    @if($user->avatar)
                                        <img src="{{ asset('storage/'.$user->avatar) }}" alt="Avatar Pengguna" class="img-thumbnail" id="previewImg">
                                    @else
                                        <img src="" alt="Preview Avatar" class="img-thumbnail d-none" id="previewImg">
                                    @endif
                                </div>
                                <small class="text-muted"><i class="bi bi-info-circle me-1"></i>Upload foto profil pengguna baru (opsional)</small>
                                @error('avatar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // If user already has avatar, update the dropzone description
            if (!previewImg.classList.contains('d-none')) {
                desc.innerHTML = `<i class="bi bi-file-earmark-check"></i><p>Foto profil sudah ada</p>`;
            }
